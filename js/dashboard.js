...
async function loadRealStats() {
  try {
    const res = await fetch(`${DASHBOARD_BACKEND_URL}/api/admin/stats`);
    if (!res.ok) throw new Error('stats fetch failed');
    const data = await res.json();

    // メトリクスカードを実データで更新
    const vals = document.querySelectorAll('.metric-val');
    if (vals[0]) vals[0].textContent = (data.total_conversations || 0).toLocaleString();
    if (vals[1]) vals[1].textContent = (data.total_users || 0).toLocaleString();
    if (vals[2]) vals[2].textContent = (data.avg_turns || 0);

    console.log('[dashboard] 実データ取得成功', data);
  } catch (err) {
    // バックエンド未起動時はダミーデータのまま表示
    console.warn('[dashboard] バックエンド未接続のためダミーデータを表示:', err.message);
  }
}
